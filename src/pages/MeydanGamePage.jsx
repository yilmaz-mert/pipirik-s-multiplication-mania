  ...
  const handleCheck = () => {
    ...
    setTimeout(() => {
      if (currentIndex + 1 < questions.length) {
        ...
        navigate('/meydan-sonuc', { state: { time: localTimer, correct: finalCorrect, wrong: finalWrong } });
      }
    }, 700);
  };
  ...
  return (
    <div className="w-full h-full relative flex flex-col items-center pb-10 overflow-hidden">
      ...
      {portalTarget && createPortal(
        <div className="w-[89.33%] max-w-83.75 flex flex-col items-center justify-center relative">
          <div className="bg-tema-kutu px-6 py-2 rounded-full shadow-sm flex items-center justify-center min-w-30">
            <span className="font-poppins font-extrabold text-[24px] text-tema-yazi tracking-widest">
              {formatTime(localTimer)}
            </span>
          </div>
          {showPenalty && (
            <div className="absolute -bottom-8 font-poppins font-extrabold text-[16px] animate-[penaltyFade_1.5s_ease-in-out_forwards]">
              +10 Sn Ceza!
            </div>
          )}
        </div>,
      ...
      {/* Soru ve Numpad alanı (Öncekiyle aynı yapı, sadece handleCheck güncellendi) */}
      <div className="relative z-20 w-[89.33%] max-w-83.75 flex flex-col items-center mt-10">
        <div 
          key={currentIndex}
